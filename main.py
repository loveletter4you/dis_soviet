...
def get_authors_pub(data, s):        
    ...
    with open('author_.txt', 'r', encoding='utf-8') as f:
            for line in f:
                ...
                get_author_pub(author_id, data, s, name, 1)
                sleep(12)
                get_author_pub(author_id, data, s, name, 2)
                sleep(14)         
                with open(f"./vak/{name}1.txt", "r", encoding="utf-8") as f:
                    text = f.read()
                    df1 = Scraper(text).start()
                    with open(f"./vak/{name}2.txt", "r", encoding="utf-8") as f1:
                          text1 = f1.read()
                          df = Scraper(text1).start()
                pd.concat([df1, df], axis=0).to_excel(f"./vak_xlsx/{name}.xlsx")
...
def get_authors_conf(d, s):
    ...
    with open('author_.txt', 'r', encoding='utf-8') as f:
        for line in f:
            name=line.split(':')[0]
            author_id=line.split(':')[1]
            get_author_conf(author_id, data, s, name, 1)
            sleep(12)
            get_author_conf(author_id, data, s, name, 2)
            sleep(14)
            get_author_conf(author_id, data, s, name, 3)
            sleep(13)          
            with open(f"./conf/{name}1.txt", "r", encoding="utf-8") as f:
                text = f.read()
                df1 = Conf_Scraper(text).start()
                with open(f"./conf/{name}2.txt", "r", encoding="utf-8") as f1:
                      text1 = f1.read()
                      df = Conf_Scraper(text1).start()
                      with open(f"./conf/{name}3.txt", "r", encoding="utf-8") as f2:
                          text2 = f2.read()
                          df2 = Conf_Scraper(text2).start()
            pd.concat([df1, df, df2], axis=0).to_excel(f"./conf_xlsx/{name}_conf.xlsx")
...
def get_core():
    ...
    wos_df = pd.read_excel('WoS-17-21.xls')
    with open('author_.txt', 'r', encoding='utf-8') as f:
        for line in f:
            name=line.split(':')[0]
            ...
            for idx,row in df.iterrows():
                if scopus_id in str(row['Scopus Author Ids']):
                    ...
                    issue = ''
                    db1.append('Scopus')
                    if row['Title'] in wos_df['Article Title'].unique():
                        db2.append('WoS')
                    else:
                        db2.append('-')
                    if row['Issue'] != '-':
